...
export default function SiteHeader() {
  ...
  useEffect(() => {
    try {
      const savedLang = localStorage.getItem('wk_lang')
  ...
  return (
    <>
          ...
          <div className="flex items-center gap-1.5 flex-shrink-0">
            ...
            {user ? (
              <div className="flex items-center gap-2">
                <span className="text-[12px] text-ink-3 hidden sm:inline-block max-w-[100px] truncate">
                  👤 {user.email || user.phone}
                </span>
                <button onClick={handleLogout}
                  className="text-xs font-medium text-ink-3 hover:text-ink px-2 py-1.5 rounded-md hover:bg-surface-2 transition-colors">
                  {lang === 'zh' ? '退出' : 'Logout'}
                </button>
              </div>
            ) : (
      ...
      {/* 登录弹窗 - 移出 header 确保 z-index 和 position:fixed 正常 */}
      {showAuth && (
        <AuthModal
